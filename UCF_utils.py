...
def sequence_generator(data_list, batch_size, input_shape, num_classes):
    ...
    :return:
    '''
    #convert inputs from tensors
    if type(input_shape) is np.ndarray:
    ...
    while True:
        ...
        for i in range(batch_size):
            step = random.randint(1, data_list.shape[0] - 1)  # approach a random-size step to get the next video sample
            index = (index + step) % data_list.shape[0]
            clip_dir, clip_class = (data_list[index,0].decode('utf-8'),int(data_list[index,1]))
            ...
            while not os.path.exists(clip_dir):
                ...
                index = (index + 1) % data_list.shape[0]
                clip_dir, class_idx = (str(data_list[index,0]),int(data_list[index,1]))
